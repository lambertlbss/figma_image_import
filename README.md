...
- 支持两种组件集分类方式：严格尺寸、AI 辅助（Figma MCP，可选）。默认使用严格尺寸，AI 辅助也以严格尺寸结果为基础进行调整。
...
- **AI 辅助（MCP）**：先生成严格尺寸候选组，再由外部 AI 客户端调整需要合并、拆分、改名或保持独立的资源。未被 AI 提及的资源继续保持严格尺寸结果；插件只接受当前扫描对应、格式合法且置信度不低于 `0.7` 的方案，跨一级目录、重复归组或引用不存在资源的方案会在修改画布前被拒绝。

### AI 辅助流程

1. 在分类方式中选择“AI 辅助（基于严格尺寸）”，先在下方勾选需要 AI 审核的文件夹，再点击“生成 MCP 请求”。插件只会把这些文件夹的资源清单、已有 Figma 节点 ID 和严格尺寸候选组写入当前页面的 shared plugin data，并复制一段提示词。
