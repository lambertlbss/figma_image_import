...
- **AI 辅助（MCP）**：同时生成严格尺寸候选组和名称模板语义候选组，再由外部 AI 客户端双向审核：拆分同尺寸异类、合并跨尺寸同系列、改名或保持独立。语义候选会识别状态、方向、品质、字形、编号/帧、倍率和一般命名系列，并携带尺寸关系与变化槽位证据。未被 AI 提及的资源继续保持严格尺寸结果；插件只接受当前扫描对应、格式合法且置信度不低于 `0.7` 的方案，跨一级目录、重复归组或引用不存在资源的方案会在修改画布前被拒绝。

### AI 辅助流程

1. 在分类方式中选择“AI 辅助（基于严格尺寸）”，先在下方勾选需要 AI 审核的文件夹，再点击“生成 MCP 请求”。插件只会把这些文件夹的资源清单、已有 Figma 节点 ID、严格尺寸候选组、跨尺寸语义候选组和审核规则写入当前页面的 shared plugin data，并复制一段提示词。
